...
- [ ] 2.1 Add a pinned pnpm workspace for `apps/mobile`, `packages/device-protocol`, `packages/device-client`, and `packages/mock-device`, with shared strict TypeScript, formatting, lint, and test configuration.
- [ ] 2.2 Scaffold the Expo/React Native/TypeScript application with Expo Router, document mock versus physical configuration, and keep screen code independent of the selected transport.
